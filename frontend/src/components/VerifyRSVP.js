...
import { verifyRSVP, updateRSVP, getMember } from "../api"; // ✅ added getMember
  ...
  const handleVerifyRSVP = async (e) => {
    ...
    try {
      ...
      if (searchMode === "confNumber") {
        ...
        // ✅ Name + House search → first get member → then fetch all RSVPs
        const member = await getMember({ name: name.trim(), houseNumber: houseNumber.trim() });
        if (!member || !member.name) {
          setError("Member not found.");
          setVerifying(false);
          return;
        }
        // We expect backend verifyRSVP can accept memberId or mememail (adjust if different)
        data = await verifyRSVP({ mememail: member.email });
      }
