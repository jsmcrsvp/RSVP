...
const AdminAddProgram = () => {
  ...
  useEffect(() => {
    const fetchPrograms = async () => {
      try {
        const res = await getAdminAllPrograms();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await addAdminNewProgram({ program_name: programName.trim() });
      setPrograms([res.data.program, ...programs]);
      setMessage(res.data.message || "Program added successfully");
      setProgramName("");
    } catch (err) {
